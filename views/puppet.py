...
@app.route('/puppet/roles', methods=['GET', 'POST'])
@cortex.lib.user.login_required
def puppet_roles():
	"""Handles the Puppet Roles page"""

	# Get a cursor to the databaserino
	curd = g.db.cursor(mysql.cursors.DictCursor)
	...
	if request.method == 'GET':
		...
		return render_template('puppet-roles.html', active='puppet', data=results, title="Puppet Roles")
#	else:
#		if request.form['action'] == 'add':
#			netgroup_name = request.form['netgroup_name']
#
